...
Red Hat Enterprise Linux版本8和9不支持Docker引擎、而这是BlueXP分类安装所必需的。现在、我们支持在使用Podman版本4或更高版本作为容器基础架构的RHEL 9.0、9.1和9.2上进行安装。如果您的环境要求使用最新版本的RHEL、现在可以在使用Podman时安装BlueXP分类(1.26或更高版本)。

目前、我们不支持非公开站点安装或分布式扫描环境(使用主扫描程序节点和远程扫描程序节点)。
...
您可以使用访问默认监视列表或信息板的Upgrade Advisor页面 link:https://activeiq.netapp.com/redirect/upgrade-advisor["Upgrade Advisor"^] 链接。如果您没有默认监视列表或信息板、单击此链接将重定向到"搜索"页面。



==== 升级计划

升级计划已进行优化、可删除冗余升级步骤、修改信息和警告并简化回退计划。集群中所有节点上的通用步骤均已整合、并可在常规信息部分下找到。要查看改进后的计划、请使用中所述的步骤生成升级计划 link:https://docs.netapp.com/us-en/active-iq/task_view_upgrade.html["使用升级计划升级ONTAP版本"]。
